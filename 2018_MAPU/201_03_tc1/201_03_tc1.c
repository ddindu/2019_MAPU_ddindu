...
interrupt [EXT_INT0] void ext_int0_isr(void)
{
    ...
    OCR1AL = 0x00;
    lcd_gotoxy(0,0);  
    sprintf(sbuf,"OCR1A = 0x%02x%02x",OCR1AH,OCR1AL);
    lcd_putsf("sbuf");
    lcd_gotoxy(0,1);
    sprintf(portc,"PORTC = 0x%02x",PORTC);
    lcd_putsf("portc");
...
interrupt [EXT_INT2] void ext_int2_isr(void)
{
    ...
    OCR1BH = 0x4E;
    OCR1BL = 0x20; 
    sprintf(sbuf,"OCR1A = 0x%02x%02x",OCR1BH,OCR1BL);
    lcd_gotoxy(0,0);
    lcd_putsf("sbuf");
    lcd_gotoxy(0,1);
    sprintf(portc,"PORTC = 0x%02x",PORTC);
...
{   
    ...
    OCR1CH = 0x9C;
    OCR1CL = 0x40;
    sprintf(sbuf,"OCR1A = 0x%02x%02x",OCR1CH,OCR1CL);
    lcd_gotoxy(0,0);
    lcd_putsf("sbuf");
    lcd_gotoxy(0,1);
    sprintf(portc,"PORTC = 0x%02x",PORTC);
...
// Timer Period: 20.001 ms
// Output Pulse(s):
// OC1A Period: 20.001 ms Width: 20.001 ms
// OC1B Period: 20.001 ms Width: 0 us
